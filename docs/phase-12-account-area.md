...
**Status (audited 2026-09-01 against the working tree):** 12.1, 12.2 and 12.3 are done and match this spec closely. 12.4, 12.5 and 12.6 still render through the **pre-Classical** components (plain list rows, old `OrderStatusStepper` icon-circle variant, generic Clerk `shadcn` theme) — functionally correct but not re-skinned to S12/S13. Per-task status is marked inline below.
