# Phase 3 — Auth (Clerk), Cart Merge

**Objective:** complete guest-first Clerk authentication for the storefront, attach fresh Bearer tokens only inside the server BFF, merge an anonymous cart into the signed-in cart exactly once, clear user-scoped browser cache on sign-out, and handle disabled accounts globally.
...
**API surface:** Auth-mode plumbing from `00-conventions.md`; the merge is triggered by authenticated `GET /cart`. There is no merge endpoint.

**Out of scope: storefront profile editing.** Clerk's own `<UserButton>` "Manage account" panel already lets a signed-in user edit name, username, email, and phone, and the backend's `user.updated` webhook (`ClerkSyncService.upsertFromWebhookUser`) already syncs those edits back into the local `users` table — so a self-hosted profile form in `/account` is redundant. `GET /users/me` and `PATCH /users/me` (`11-profile.md`) remain documented backend endpoints but are not wired up by this storefront. `/account` is a thin placeholder until Phase 4 ships addresses/orders.
...
Storefront profile editing (see above — Clerk's own account panel covers it), wishlist, addresses, order history/detail, reviews management, checkout, and order tracking. Their optional entry points may use `<RequireAuth>`, but their screens ship in later phases.
