...
| 12 | [phase-12-account-area.md](./phase-12-account-area.md) | **in progress** — 12.1 (S10 overview), 12.2 (profile), 12.3 (S11 addresses), and 12.4 (S12 orders) done in code; the 12.3 and 12.4 browser checklists remain pending; 12.3 also extracted `features/shipping/` and moved the addresses panel into URL state; 12.5 (S13 order detail) and 12.6 (wishlist, guest tracking, Clerk auth screens) remain pre-Classical | S10 Overview (new), S11 Addresses, S12 Orders, S13 Order detail, profile, wishlist | 04, 07, 08, 10, 11 |
