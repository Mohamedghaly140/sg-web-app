...
| 10 | [phase-10-bag-and-checkout.md](./phase-10-bag-and-checkout.md) | **in progress** — all tasks 10.1–10.6 implemented; Definition of Done (end-to-end CASH order verification for both flows) not yet re-run; 10.5 also built the `order-track-lookup` widget and `/orders/track` stub ahead of schedule | S4 Cart, S5 registered, S6 guest, S7 confirmation, shared step rail | 05, 06, 07, 09 |
| 11 | [phase-11-contact-and-order-help.md](./phase-11-contact-and-order-help.md) | **done** — 11.1–11.4 landed: route, atelier and order-help column, validated mail and WhatsApp handoffs with no false success state, shipping fallback links, GAP-5 fallback record, and the Phase 12 destination note | S9 Contact — new route and feature; order-help lookup | 10 (guest lookup) |
| 12 | [phase-12-account-area.md](./phase-12-account-area.md) | **in progress** — 12.1–12.6 all done in code, including the S13 order-detail re-skin shared safely with guest tracking and the 12.6 wishlist S2 grid, Classical Clerk `appearance` and S11/S12 `h1` fix; the 12.3–12.5 browser checklists remain pending, as do 12.6's wishlist checklist and the `/orders/track/[token]` verification; 12.3 also extracted `features/shipping/` and moved the addresses panel into URL state | S10 Overview (new), S11 Addresses, S12 Orders, S13 Order detail, profile, wishlist | 04, 07, 08, 10, 11 |
